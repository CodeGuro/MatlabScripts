function [ vecMistakes_avg, vecMistakes_dev  ] = sample_numMistakes_sigma( n, steady_vecX, matA, vecB, matK, matN, setsJ, powerSetsJ, ...
    ...
    matdeltaX = nan( n, n*num_samples );

    for sample_num = 1:num_samples

        disp(['iterating... sample ' num2str(sample_num) ' out of ' num2str(num_samples)]);

        matdeltaX( :, ((sample_num-1)*n+1): (sample_num*n) ) = construct_matdeltaX( n, steady_vecX, vecB, matK, matN, setsJ, powerSetsJ, ...
...
%         end
    ...
    matK_rec = matK_rec_useInv( n, matdeltaX, num_samples );
    vecMistakes_inv( 1 ) = nnz( logical( matK ) - logical( abs(matK_rec) > mistake_threshold ) );
    
    if length(lambdas) > 0
        matK_recs_lasso = matK_rec_useLasso( n, matdeltaX, num_samples, lambdas, use_lasso_Nmat );
        for z=1:length(lambdas)
            numMistakes_lasso = nnz( logical( matA ) - logical( abs( matK_recs_lasso(:,:,z) ) > mistake_threshold ) );
